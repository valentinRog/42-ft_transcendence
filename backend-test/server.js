...
function update() {
    ...
    if (ball.x < wallLeft && ball.dx < 0) {
        ...
        const angle = (ball.y - (paddles[0] + dimensions.paddleHeight / 2)) /
        ...
        const angle = (ball.y - (paddles[1] + dimensions.paddleHeight / 2)) /
            (dimensions.paddleHeight / 2);
        ball.dx = -1;
        ball.dy = 0;
        const { x, y } = rotate(ball.dx, ball.dy, angle * maxAngle);
    ...
    else if (ball.y > dimensions.height - dimensions.ballRadius &&
        ball.dy > 0) {
